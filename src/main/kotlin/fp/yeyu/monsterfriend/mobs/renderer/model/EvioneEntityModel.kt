...
class EvioneEntityModel(
    scale: Float,
    pivotY: Float,
    textureWidth: Int,
    textureHeight: Int
) : CompositeEntityModel<Evione>(),
    ...
    override fun setAngles(
        ...
        val state = evione.getState()
        if (state == Evione.State.SPELL_CASTING) {
            rightAttackingArm.pivotZ = 0.0f
            rightAttackingArm.pivotX = -5.0f
            leftAttackingArm.pivotZ = 0.0f
            leftAttackingArm.pivotX = 5.0f
            rightAttackingArm.pitch = MathHelper.cos(h * 0.6662f) * 0.25f
            leftAttackingArm.pitch = MathHelper.cos(h * 0.6662f) * 0.25f
            rightAttackingArm.roll = 2.3561945f
            leftAttackingArm.roll = -2.3561945f
            rightAttackingArm.yaw = 0.0f
            leftAttackingArm.yaw = 0.0f
        }
        val bl = state == Evione.State.CROSSED
        arms.visible = bl
